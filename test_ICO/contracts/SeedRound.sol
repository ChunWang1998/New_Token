...
contract SeedRound is StandardToken{
    ...
    uint256 constant public SECONDS_PER_LOCK_PERIOD = 30 days; 
    uint256 constant public VESTED_PERIOD = 12;
    ...
    mapping(address=>uint) public RBT_balances;//for RBT, lock for one year(locked)
    bool public RBT_balances_available;
    mapping(address=>uint) public RBT_balances_per_vested;//for vested

    mapping(address=>uint) public RBT_staking;//can use after SeedRound end(locked)
    mapping(address=>uint) public per_RBT_staking;//divide RBT_staking into 365 

    mapping(address=>uint) public Available_RBT;//the RBT which can withdraw to metamask(unlocked)
    
    mapping (address=>uint)public balances;//for eth
    ...
    constructor() public{
        ...
        saleStart = now;
        staking_time = now;
        ETHFeed = AggregatorV3Interface(0x9326BFA02ADD2366b30bacB125260Af641031331);//kovan
        //ETHFeed = AggregatorV3Interface(0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419);mainNet
        BTCFeed = AggregatorV3Interface(0x6135b13325bfC4B00278B4abC5e20bbce2D6580e);//kovan
...
//     }
    ...
    function Invest()public payable{
        ...
        uint256 user_RBT = msg.value.div(10**uint(18)).mul(10).mul(10);//msg.value : wei
        ...
        require(RBT_balances[msg.sender]+user_RBT<2000000,"Purchase limit exceeded");
        if((user_RBT>100000&&Not_First_buy[msg.sender]==false) || (Not_First_buy[msg.sender]==true)){
            ...
            per_RBT_staking[msg.sender] = RBT_balances[msg.sender].mul(SEEDROUND_STAKING_PERCENTAGE).div(100);

            if(Not_First_buy[msg.sender]==false)//avoid same user be pushed several times
    ...
    function Run_per_RBT_staking ()public onlyAdmin{
        ...
        for (uint i=0; i<arrayLength; i++) {
            RBT_staking[addressIndices[i]] = RBT_staking[addressIndices[i]].add( per_RBT_staking[addressIndices[i]].div(365) );//divide it to 365
        }
    }
    ...
    function EndSeedRound()public onlyAdmin{//after public sale(about 1 year)(cliff start)
        saleEnd = now; 
        uint arrayLength = addressIndices.length;
        for (uint i=0; i<arrayLength; i++) {//move RBT from RBT_staking to Available_RBT
            Available_RBT[addressIndices[i]] =  RBT_staking[addressIndices[i]];
            RBT_staking[addressIndices[i]] = 0;
        }
    }

    function vested()public onlyAdmin{//cliff end and vested start
        uint arrayLength = addressIndices.length;
        if (RBT_balances_available == false){
            ...
            for (uint s=0; s<arrayLength; s++) {//store the RBT_balances_per_vested
                RBT_balances_per_vested[addressIndices[s]] = RBT_balances[addressIndices[s]].div(VESTED_PERIOD);
            }
        }
        else{//vested per period
            if(now>=vested_record+SECONDS_PER_LOCK_PERIOD && vested_period_cnt<VESTED_PERIOD ){
                ...
                for (uint i=0; i<arrayLength; i++) {//move RBT from RBT_balances to Available_RBT
                    Available_RBT[addressIndices[i]] = Available_RBT[addressIndices[i]].add( RBT_balances[addressIndices[i]].div(VESTED_PERIOD) );
                    RBT_balances[addressIndices[i]] = RBT_balances[addressIndices[i]].sub( RBT_balances_per_vested[addressIndices[i]] );
                }
            }
        }
    }
...
//---------------------------------------------------------------------------------------------------------------------
    ...
    function renew_percentage() public{// percentage = 20->20%
        percentage = User_totalRBT.mul(100).div(initial_totalRBT);
    }

    function Check_my_RBT()public view returns(uint){
